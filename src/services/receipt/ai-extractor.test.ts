...
function makeChatResponse(content: string | null): Response {
  return new Response(
    JSON.stringify({
      id: 'test-id-123',
...
function mockFetchWithContent(chatContent: string | null): void {
  globalThis.fetch = mock(async (url: string | URL): Promise<Response> => {
    ...
    if (urlStr.includes('/api/models/')) {
      // Extract provider and model from the URL
      const match = urlStr.match(/\/api\/models\/([^?]+)/);
      const modelId = match?.[1] ?? 'unknown/model';
      // Use the provider from the model ID prefix (e.g. "deepseek-ai" for DeepSeek)
      return makeProviderMappingResponse('fireworks-ai', modelId);
    }
