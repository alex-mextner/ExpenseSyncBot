...
/**
 * Find similar categories (returns multiple matches, sorted by score)
 */
export function findSimilarCategories(
  ...
  for (const category of categories) {
    ...
    if (normalizedCategory === normalizedInput) {
      ...
      const similarity = calculateSimilarity(phoneticInput, normalizePhonetic(category));
      if (similarity >= FUZZY_THRESHOLD) {
        score = Math.round(similarity * 60); // maps 0.9–1.0 → 54–60
      } else {
        // Word-based fallback
